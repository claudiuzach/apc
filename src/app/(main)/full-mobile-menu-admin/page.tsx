...
import { Bookmark, Clipboard, Home, MapPin, Users, X } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function FullMobileMenuAdmin() {
  ...
  return (
    <div className="fixed inset-0 bg-white dark:bg-gray-900 flex flex-col items-center justify-center space-y-6 p-6">
      ...
      <nav className="flex flex-col space-y-6 w-full max-w-md">
        ...
        {/* Admin-Only Section */}
        <Link href="/approvals" className="flex items-center gap-4 text-lg font-semibold text-red-500 dark:text-red-400">
          <Users className="w-8 h-8" />
          Approvals
        </Link>

        <Link href="/certificates" className="flex items-center gap-4 text-lg font-semibold text-red-500 dark:text-red-400">
          <Users className="w-8 h-8" />
          Membership
        </Link>
      </nav>
